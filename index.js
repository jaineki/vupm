...
// Update profile — username changes are limited to once every 7 days
app.patch('/api/auth/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.lastUsernameChangeAt) {
      ...
      if (elapsed < WEEK_MS) {
        const nextAllowed = new Date(new Date(user.lastUsernameChangeAt).getTime() + WEEK_MS);
        return res.status(429).json({
          success: false,
          message: `You can change your username again on ${nextAllowed.toLocaleDateString()}`,
          nextUsernameChangeAt: nextAllowed.toISOString()
        });
      }
    }

    const existing = await db.collection('chat_users').findOne({
      usernameLower,
      _id: { $ne: user._id }
    });
    if (existing) {
    ...
    await db.collection('chat_users').updateOne(
      { _id: user._id },
      { $set: { username: cleanUsername, usernameLower, lastUsernameChangeAt: nowIso } }
    );

    const updatedUser = await db.collection('chat_users').findOne({ _id: user._id });
    const token = signToken(updatedUser);

    res.json({ success: true, message: "Username updated successfully", token, user: sanitizeUser(updatedUser) });
...
app.get('/api/videos/:id/stats', optionalAuth, async (req, res) => {
  try {
    if (!isMongoConnected()) {
      return res.json({ success: true, views: 0, likeCount: 0, dislikeCount: 0, userLiked: false, userDisliked: false, likers: [] });
    }
    ...
    res.json({
      ...
      userLiked: !!(req.userId && likedBy.some(u => u.userId === req.userId)),
      userDisliked: !!(req.userId && dislikedBy.some(u => u.userId === req.userId)),
      likers: likedBy.map(u => u.username)
    });
  } catch (error) {
...
app.post('/api/videos/:id/like', authenticateToken, async (req, res) => {
  try {
    ...
    const alreadyLiked = stats && stats.likedBy && stats.likedBy.some(u => u.userId === userId);
    ...
    if (alreadyLiked) {
      ...
      await db.collection('video_stats').updateOne(
        { fileId },
        {
          $addToSet: { likedBy: { userId, username } },
    ...
    res.json({
      ...
      likeCount: likedBy.length,
      dislikeCount: dislikedBy.length,
      userLiked: likedBy.some(u => u.userId === userId),
      userDisliked: dislikedBy.some(u => u.userId === userId),
      likers: likedBy.map(u => u.username)
    });
...
app.post('/api/videos/:id/dislike', authenticateToken, async (req, res) => {
  try {
    ...
    let stats = await db.collection('video_stats').findOne({ fileId });
    const alreadyDisliked = stats && stats.dislikedBy && stats.dislikedBy.some(u => u.userId === userId);
    ...
    if (alreadyDisliked) {
      ...
      await db.collection('video_stats').updateOne(
        { fileId },
        {
          $addToSet: { dislikedBy: { userId, username } },
          $pull: { likedBy: { userId } },
          $setOnInsert: { views: 0 }
        },
        { upsert: true }
...
io.on("connection", async (socket) => {
  ...
  socket.on("message:send", async (data) => {
    ...
    if (trimmedMessage.toLowerCase().startsWith("/ai ")) {
      ...
      try {
        // Call the AI command module (commands/ai.js)
        const aiReply = await aiCommand.run(prompt);
    ...
    // ============================================
    // /music COMMAND — searches iTunes and sends back
    // a playable preview clip + cover art in the chat
    // ============================================
    else if (trimmedMessage.toLowerCase().startsWith("/music ")) {

      const query = trimmedMessage.substring(7).trim();
      ...
      if (!query) {
        const errorMsg = {
          ...
          userId: "ai-bot",
          username: "🎵 Music Bot",
          message: "⚠️ Please provide a song name. Example: `/music umaasa`",
          timestamp: new Date().toISOString()
        };
        messages.push(errorMsg);
        io.emit("message:new", errorMsg);
      ...
      io.emit("typing:start", { userId: "ai-bot", username: "🎵 Music Bot" });

      try {
        const song = await musicCommand.run(query);
        ...
        if (isMongoConnected()) {
          db.collection('chat_messages').insertOne(musicMessage).catch(err =>
            console.error('Save music message error:', err)
          );
        }

        io.emit("message:new", musicMessage);
        ...
        const errorMessage = {
          ...
          userId: "ai-bot",
          username: "🎵 Music Bot",
          message: `⚠️ ${err.message || "Couldn't find or play that song. Try a different name."}`,
          timestamp: new Date().toISOString()
        };
        ...
        if (isMongoConnected()) {
          db.collection('chat_messages').insertOne(errorMessage).catch(err =>
            console.error('Save music error message error:', err)
          );
        }

        io.emit("message:new", errorMessage);
      } finally {
        io.emit("typing:stop", { userId: "ai-bot", username: "🎵 Music Bot" });
      }
    }

    // ============================================
    // /song COMMAND — same iTunes-based search as /music,
    // sent as a separate command per request
    // ============================================
    else if (trimmedMessage.toLowerCase().startsWith("/song ")) {
      ...
      if (!query) {
        const errorMsg = {
          ...
          userId: "ai-bot",
          username: "🎶 Song Bot",
          message: "⚠️ Please provide a song name. Example: `/song umaasa`",
          timestamp: new Date().toISOString()
        };
        messages.push(errorMsg);
        io.emit("message:new", errorMsg);
      ...
      io.emit("typing:start", { userId: "ai-bot", username: "🎶 Song Bot" });

      try {
        const song = await songCommand.run(query);
        ...
        messages.push(songMessage);
        if (messages.length > MAX_MESSAGES_STORED) {
          messages = messages.slice(-MAX_MESSAGES_STORED);
        }

        if (isMongoConnected()) {
          db.collection('chat_messages').insertOne(songMessage).catch(err =>
            console.error('Save song message error:', err)
          );
        }

        io.emit("message:new", songMessage);
        ...
        if (isMongoConnected()) {
          db.collection('chat_messages').insertOne(errorMessage).catch(err =>
            console.error('Save song error message error:', err)
          );
        }

        io.emit("message:new", errorMessage);
      } finally {
        io.emit("typing:stop", { userId: "ai-bot", username: "🎶 Song Bot" });
      }
    }
  ...
  socket.on("message:react", async (data) => {
    ...
    if (msg.reactions[user.userId] && msg.reactions[user.userId].emoji === emoji) {
      delete msg.reactions[user.userId];
    } else {
      msg.reactions[user.userId] = { emoji, username: user.username };
    }

    if (isMongoConnected()) {
      db.collection('chat_messages').updateOne(
        { id: messageId },
        { $set: { reactions: msg.reactions } }
      ).catch(err => console.error('Save reaction error:', err));
    }
  ...
  // ============================================
  // MESSAGE UNSEND (only the message's own author can unsend it)
  // ============================================
  socket.on("message:unsend", async (data) => {
    ...
    if (msg.userId !== user.userId) {
      socket.emit("error", { message: "You can only unsend your own messages" });
      return;
    }
    ...
    if (isMongoConnected()) {
      db.collection('chat_messages').deleteOne({ id: messageId }).catch(err =>
        console.error('DB unsend message error:', err)
      );
    }

    io.emit("message:unsent", { messageId, userId: user.userId });
  });

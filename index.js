...
io.on("connection", async (socket) => {
  ...
  socket.on("message:send", async (data) => {
    ...
    // ============================================
    // /music COMMAND — searches SoundCloud/YouTube and
    // sends back a playable audio message in the chat
    // ============================================
      ...
      if (!query) {
        const errorMsg = {
          ...
          userId: "ai-bot",
          username: "🎵 Music Bot",
          message: "⚠️ Please provide a song name. Example: `/music Shape of You`",
          timestamp: new Date().toISOString()
        };
        messages.push(errorMsg);
        io.emit("message:new", errorMsg);
      ...
      try {
        ...
        if (scResult && scResult.status && Array.isArray(scResult.data) && scResult.data.length > 0) {
          ...
          const ytRes = await axios.get(
            `https://nayan-video-downloader.vercel.app/ytsearch?query=${encodeURIComponent(query)}`,
            { timeout: 15000 }
          );
          results = (ytRes.data?.data || ytRes.data?.results || []).slice(0, 6);
          source = "youtube";
        }

        if (!results.length) throw new Error("No songs found for that search.");

        const top = results[0];
        const title = top.title || top.name || top.song || query;
        const artist = top.artist || top.author || top.uploader || "";
        const trackUrl = top.url || top.link || top.permalink;
        ...
        if (!streamUrl) throw new Error("Could not get a playable link for that song.");
        ...
        const errorMessage = {
          ...
          userId: "ai-bot",
          username: "🎵 Music Bot",
          message: "⚠️ Couldn't find or play that song. Try a different name.",
          timestamp: new Date().toISOString()
        };
